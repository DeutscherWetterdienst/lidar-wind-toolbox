...
def import_lvl1(date_chosen: datetime.datetime, confDict: dict[str, str]) -> xr.Dataset:
    ...
    mylist = list(
        path.glob(
            "**/" + confDict["NC_L1_BASENAME"] + "*" + date_chosen.strftime("%Y%m%d") + "*.nc"
        )
    )
    if len(mylist) > 1:
...
def import_lvl2(date_chosen: datetime.datetime, confDict: dict[str, str]) -> xr.Dataset:
    ...
    mylist = list(
        path.glob(
            "**/" + confDict["NC_L2_BASENAME"] + "*" + date_chosen.strftime("%Y%m%d") + "*.nc"
        )
    )
    if len(mylist) > 1:
        print("!!!multiple files found!!!, only first is processed!")
...
class hpl2netCDFClient(object):
    ...
    def dailylvl1(self) -> None:
        ...
        hpl_listm1 = hpl_files.make_file_list(
            date_chosen + datetime.timedelta(minutes=-30), confDict, url=confDict["PROC_PATH"]
        )
        print("looking at the following day")
        hpl_listp1 = hpl_files.make_file_list(
            date_chosen + datetime.timedelta(days=+1, minutes=30),
            confDict,
            url=confDict["PROC_PATH"],
        )
    ...
    def lvl2_from_filelist(
        ...
        # to output file
        timestamp_out = files_hpl.time[0].strftime(
            "%Y%m%d%H%M"
        )  # set stamp of output file to stamp of first infile
        aux_fn_info = ""
    ...
    def lvl2ql(self) -> None:
        ...
        if np.all(np.isnan(U)):
            ...
            dp1 = (
                datetime.datetime(d0.year, d0.month, d0.day)
            ...
            ax.xaxis.set_minor_locator(
                mdates.HourLocator(byhour=np.mod(range(0 - time_delta, 24 - time_delta, 1), 24))
            )
            # put x-and y-label
            ...
            print(
                "saving wind retrieval quicklook as... \n ... "
                + str(
                    "{}/"
                    + confDict["NC_L2_BASENAME"]
                    + "ql_"
    ...
    def bckql(self) -> None:
        ...
        if np.all(np.isnan(Z)):
            ...
            if condi:
                ...
                c_temp = ax.pcolormesh(X.T, Y.T, masked_Z, cmap=cm.gnuplot2, vmin=vmin, vmax=vmax)
            else:
                str_bck = r"$\rm{attenuated\;backscatter}\;/\;\rm{m}^{-1}\,\rm{sr}^{-1}$"
                c_temp = ax.pcolormesh(
                    X.T, Y.T, masked_Z, cmap=cm.gnuplot2, norm=mcolors.LogNorm(vmin=vmin, vmax=vmax)
                )
            ...
            print(
                "saving backscatter quicklook as... \n ... "
                + str(
                    "{}"
                    + confDict["NC_L2_BASENAME"]
                    + "bck_ql_"
    ...
    def nrtlvl1(self) -> None:
        ...
        start, end = (
            time_chosen - datetime.timedelta(minutes=delt + 3),
            time_chosen + datetime.timedelta(minutes=3),
        )
        ...
        namelist = list(
            map(
                hpl_list.name.__getitem__,
                np.where(
                    (np.array(hpl_list.time) >= start.replace(tzinfo=None))
                    & (np.array(hpl_list.time) <= end.replace(tzinfo=None))
                )[0],
            )
        )
        timelist = list(
            map(
                hpl_list.time.__getitem__,
                np.where(
                    (np.array(hpl_list.time) >= start.replace(tzinfo=None))
                    & (np.array(hpl_list.time) <= end.replace(tzinfo=None))
                )[0],
        ...
        namelist = list(
            map(
                ...
                np.where(
                    (np.array(timelist) >= start.replace(tzinfo=None))
                    & (np.array(timelist) <= end.replace(tzinfo=None))
                )[0],
            )
        )
        timelist = list(
            map(
                timelist.__getitem__,
                np.where(
                    (np.array(timelist) >= start.replace(tzinfo=None))
                    & (np.array(timelist) <= end.replace(tzinfo=None))
                )[0],
    ...
    def rmlvl1(self) -> None:
        ...
        if "UTC_OFFSET" in confDict:
            ...
            time_delta = 0
        time_chosen = self.date2proc + datetime.timedelta(hours=time_delta)
        path = Path(confDict["NC_L1_PATH"]) / Path(
    ...
    def nrtlvl2(self) -> None:
        ...
        width = (
            ds_tmp.range.data * 2 * np.cos(np.nanmedian(elevation[elevation < 89]) * np.pi / 180)
        )
        ...
        if len(time_ds) != 0:
            time_bnds = np.array(
                [
                    [
                        int(
                            pd.to_datetime(time_ds[0])
                            .replace(tzinfo=datetime.timezone.utc)
                            .timestamp()
                        ),
                        int(
                            pd.to_datetime(time_ds[-1])
                            .replace(tzinfo=datetime.timezone.utc)
                            .timestamp()
                        ),
                    ]
        ...
        if len(time_ds) != 0:
            ...
            if ~indicator:
                ...
                UVW[0, np.sum(~np.isnan(VR_CNSmax.T), axis=1) < 4, :] = np.squeeze(
                    np.full((3, 1), np.nan)
                )
                ...
                UVWunc[0, ...] = abs(
                    np.einsum(
                        ...
                        np.sqrt(
                            (
                                A_r_MP @ np.apply_along_axis(np.diag, 1, SIGMA_r**2) @ A_r_MP_T
                            ).astype(complex)
                        ).real,
                    )
                )
                # plausible winds can only be calculated, when the at least three LOS measurements are present
                UVWunc[0, np.sum(~np.isnan(VR_CNSmax.T), axis=1) < 4, :] = np.squeeze(
                    np.full((3, 1), np.nan)
                )
                ...
                SPEED[0, :], SPEEDunc[0, :] = np.vstack(
                    [
                        ...
                        for val, unc in zip(UVW[0, ...], UVWunc[0, ...])
                    ]
                ).T
                DIREC[0, :], DIRECunc[0, :] = np.vstack(
                    [
        ...
        if "NC_INSTRUMENT_FIRMWARE_VERSION" in confDict:
            ds_lvl2.attrs["instrument_firmware_version"] = confDict[
                "NC_INSTRUMENT_FIRMWARE_VERSION"
            ]
        else:
        ...
        if "OPERATIONAL" in confDict:
            if bool(int(confDict["OPERATIONAL"])):
                ...
                ds_lvl2.attrs["references"] = confDict[
                    "NC_REFERENCES"
                ]  #'Doppler lidar PPI-based retrieval, see VAD'
